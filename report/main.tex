...
To do so, we investigate the NOTEARS algorithm (TODO reference) and the DAG-GNN algorithm (TODO)
compared to the classical algorithms PC, FGES, and hill climbing. We selected NOTEARS because
...
The blearn library (todo reference https://www.bnlearn.com/bnrepository/) contains a repository of 
prebuilt Bayesian networks that can be used to generate a synthetic dataset, with DAGs ranging from 
just 5 variables to 1041. To avoid only using synthetic data we also tested on the protein folding
dataset introduced in (TODO cite Sachs et al.) which contains a DAG with 11 nodes and a dataset of
...
only. For more details on the networks we refer the reader to the website: TODO.\\
...
and ALARM (37 nodes) networks from the bnlearn repository and generated a synthetic dataset of 1000
samples for these. We also used the (TODO cite Sachs et al) dataset, containing 11 nodes and 11672
real samples. We also note that in the preliminary phase the BARLEY graph from the bnlearn repository
...
more time to terminate and DAG-GNN took 16 times. Table 1 contains the performance of each algorithm
