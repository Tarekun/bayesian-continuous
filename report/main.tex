...
Table 1 contains the average performance of each algorithm across all datasets.

\begin{table}[t]
\centering
\footnotesize
\setlength{\tabcolsep}{4pt}
\caption{Results for each algorithm averaged across all four datasets (Sachs, Cancer, Child, Alarm). \emph{nSHD} = SHD\,/\,$|E^*|$ (SHD normalised by the number of true edges), making cross-network averages comparable. \emph{Acc.} = TP\,/\,(TP\,+\,FP\,+\,FN\,+\,rev.). FGES was not run on ALARM; its average excludes that network.}
\label{tab:results}
\vspace{4pt}
\begin{tabular}{lrrrrrr}
\toprule
Algorithm & Time (s) & nSHD & Acc. & Prec. & Rec. & F1 \\
\midrule
